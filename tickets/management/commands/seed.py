...
from tickets.services import LevelService, PointService
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if MemberLevel.objects.exists():
            self.stdout.write("会员种子数据已存在，跳过")
        ...
        self.stdout.write("会员系统种子数据初始化完成")
    ...
    def _seed_point_rules(self):
        rules = [
            PointRule.objects.create(
                source="purchase", name="购票消费", points=0, per_yuan=1,
                expire_days=365, daily_limit=0, enabled=True,
                description="每消费1元得1积分",
            ),
            PointRule.objects.create(
                source="review", name="演出评价", points=20, per_yuan=0,
                expire_days=180, daily_limit=5, enabled=True,
                description="每次评价得20积分，每日上限5次",
            ),
            PointRule.objects.create(
                source="checkin", name="每日签到", points=5, per_yuan=0,
                expire_days=90, daily_limit=1, enabled=True,
                description="每日签到得5积分",
            ),
            PointRule.objects.create(
                source="activity", name="活动奖励", points=0, per_yuan=0,
                expire_days=365, daily_limit=0, enabled=True,
                description="参与活动获得积分",
            ),
            PointRule.objects.create(
                source="birthday", name="生日礼", points=100, per_yuan=0,
                expire_days=365, daily_limit=0, enabled=True,
                description="生日当天领取生日礼积分",
            ),
            PointRule.objects.create(
                source="refund_return", name="退票返还", points=0, per_yuan=0,
                expire_days=365, daily_limit=0, enabled=True,
                description="退票时返还已使用的积分",
            ),
            PointRule.objects.create(
                source="admin", name="管理员调整", points=0, per_yuan=0,
                expire_days=365, daily_limit=0, enabled=True,
                description="管理员手动调整积分",
            ),
        ]
        self.stdout.write(f"已创建 {len(rules)} 条积分规则")
    ...
    def _seed_members(self):
        ...
        for md in members_data:
            ...
            member = Member.objects.create(
                name=md["name"], phone=md["phone"], gender=md["gender"],
                birthday=bday, level=md["level"],
            ...
            LevelChangeLog.objects.create(
                member=member, direction="initial", to_level=md["level"],
        ...
        for member in members:
            ...
            if earn_points > 0:
                ...
                PointTransaction.objects.create(
                    ...
                    balance_after=member.points_balance + 500,
                    source="purchase", source_ref="seed", batch=batch,
                    remark="种子数据-购票累计积分",
                    created_at=now - timedelta(days=100),
                )
                count += 1
            if member.total_points_spent > 0:
                PointTransaction.objects.create(
                    member=member, type="spend", points=-member.total_points_spent,
                    balance_after=member.points_balance,
                    source="mall_exchange", source_ref="seed", batch=batch,
                    remark="种子数据-商城兑换消耗积分",
                    created_at=now - timedelta(days=30),
                )
        self.stdout.write(f"已创建 {count} 条积分流水")
    ...
    def _seed_mall_products(self):
        ...
        products = [
            ...
            MallProduct.objects.create(
                ...
                point_price=800, stock=50, min_level=levels[2],
            ...
            MallProduct.objects.create(
                ...
                point_price=5000, stock=10, min_level=levels[2],
                limit_per_member=1, enabled=True,
            ),
        ]
        self.stdout.write(f"已创建 {len(products)} 件商城商品")

    def _seed_priority_quotas(self):
        perfs = Performance.objects.filter(priority_start_at__isnull=False)
        levels = MemberLevel.objects.filter(priority_hours__gt=0)
        count = 0
        for perf in perfs:
            for lv in levels:
                PerformancePriorityQuota.objects.get_or_create(
